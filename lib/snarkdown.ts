const TAGS:any = {
  ...
  export function parse(md: string, prevLinks?: { [key: string]: string }): string {
    let tokenizer = /((?:^|\n+)(?:---+|\* \*(?: \*)+)|(?:^``` *(\w*)\n([\s\S]*?)```$)|((?:(?:^|\n+)(?:\t| {2,}).+)+)|((?:(?:^|\n)([>*+-]|\d+\.)\s+.*)+)|(?:!\[([^\]]*?)\]\(([^)]+?)\))|(\[)|(\](?:\(([^)]+?)\))?)|(?:(?:^|\n+)([^\s].*)(-{3,}|={3,})(?:\n+|$))|(?:(?:^|\n+)(#{1,6})\s*(.+)(?:\n+|$))|(?:`([^`].*?)`)|(  *\n{2,}|__|\*\*|[_*]|~~))/gm;
    let context: any = [];
    let out: string = '';
    let links: { [key: string]: string } = prevLinks || {};
    let last: number = 0;
    let chunk: any, prev: string, token: any, inner: string, t: string;
  
    function tag(token: any): string | RegExpExecArray {
      let desc = TAGS[token[1] || ''];
      let end:any = context[context.length - 1] == token;
